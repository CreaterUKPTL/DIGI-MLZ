...
def short_url(longurl, attempt=0):
    ...
    try:
        if "shorte.st" in _shortener:
            ...
            return cget('GET', f'http://api.shareus.io/easy_api?key={_shortener_api}&link={longurl}').text
        else:
            res = cget('GET', f'https://{_shortener}/api?api={_shortener_api}&url={longurl}').json()
